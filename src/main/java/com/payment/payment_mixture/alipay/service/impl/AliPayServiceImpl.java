package com.payment.payment_mixture.alipay.service.impl;
...
import com.payment.payment_mixture.alipay.service.IAliPayService;
import com.payment.payment_mixture.alipay.util.AliPayConfig;
import com.payment.payment_mixture.constants.Constants;
import com.payment.payment_mixture.model.Product;
import com.payment.payment_mixture.util.CommonUtil;
...
//		String returnUrl = "http://192.168.1.113:8080/payment_mixture/frontRcvResponse";
		alipayRequest.setReturnUrl(return_url);//前台通知
        alipayRequest.setNotifyUrl(notify_url);//后台回调
        JSONObject bizContent = new JSONObject();
        bizContent.put("out_trade_no", product.getOutTradeNo());
	...
	@Override
	public String aliQuery(Product product) {
		logger.info("支付宝支付订单查询");
		String orderString = Constants.FAIL;
		AlipayClient alipayClient = AliPayConfig.getAlipayClient();
		AlipayTradeQueryRequest request = new AlipayTradeQueryRequest();
		AlipayTradeQueryModel model = new AlipayTradeQueryModel();
		model.setOutTradeNo(product.getOutTradeNo());
		if(!StringUtils.isEmpty(product.getTradeNo())){
			model.setTradeNo(product.getTradeNo());
		}
        request.setBizModel(model);
		try {
			// 这里和普通的接口调用不同，使用的是sdkExecute
